...
launchDockerBuild() {
    ...
    sh -c "docker build ${DOCKER_BUILD_ARGS} -f ${DOCKERFILE_NAME}${TARGET_OPTS}${TAGS_OPTS} ."
}
...
if [[ "$1" = "noproxy" ]]; then
    ...
    echo "Proxy mode"
    DOCKER_BUILD_ARGS="--build-arg http_proxy=$http_proxy --build-arg https_proxy=$https_proxy --build-arg HTTP_PROXY=$HTTP_PROXY --build-arg HTTPS_PROXY=$HTTPS_PROXY --build-arg APT_ARG_1=\"-o Acquire::https::Verify-Host=false\" --build-arg APT_ARG_2=\"-o Acquire::https::Verify-Peer=false\""
fi
